...
for p in data:
    ...
    # [사전 처리 1] 전화번호 및 아이콘 태그
    # 내선번호(T) 처리
    t_tag = ""
    ext_val = p.get('ext', '')
    if ext_val:
        ext_num = f"022258{ext_val}" if p['name'] == "주상건" else f"023147{ext_val}"
        t_tag = f'<a href="tel:{ext_num}" class="icon-link">T</a>'
    
    # 휴대폰(M) 처리
    m_num = p.get('mobile', '').replace('-', '')
    m_tag = f'<a href="tel:{m_num}" class="icon-link">M</a>' if m_num else ""
    
    # [사전 처리 2] 텍스트 정보 (직함, 부서, 업무)
    pos = p.get('pos', '')
    dept = p.get('dept', '')
    sep = " · " if pos and dept else ""
    work = f"- {p['work']}" if p.get('work') else ""

    # [최종 출력] 
    # f-string 내부에서 로직을 최소화하여 HTML 깨짐 방지
    html_content = f"""
