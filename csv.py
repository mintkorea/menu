...
def get_search_corpus(row):
    # 이름, 호실, 설명(교수명 등), 카테고리를 하나의 검색용 텍스트로 합침
...
def main():
    ...
    # CSS 스타일 업데이트 (description 가독성 향상)
    st.markdown("""
        <style>
        ...
        .name-line { font-weight: 600; color: #333; font-size: 0.98rem; display: flex; align-items: center; }
        .room-tag { color: #d63384; font-size: 0.85rem; margin-left: 6px; background: #fff0f6; padding: 0 5px; border-radius: 3px; }
        .desc-line { color: #666; font-size: 0.82rem; margin-top: 4px; line-height: 1.4; border-left: 2px solid #ddd; padding-left: 8px; }
        </style>
    """, unsafe_allow_html=True)

    st.markdown("<div class='m-title'>🏥 성의교정 통합 안내 (교수실/시설 검색)</div>", unsafe_allow_html=True)
    ...
    if data is not None:
        ...
        for _, row in view_df.iterrows():
            ...
            # UI 구성
            room_html = f"<span class='room-tag'>{room_val}호</span>" if room_val and room_val != 'nan' else ""
            desc_html = f"<div class='desc-line'>{desc_val}</div>" if desc_val and desc_val != 'nan' and desc_val.strip() != "" else ""
            
            st.markdown(f"""
                <div class='result-row'>
                    <div class='loc-tag'>{floor_disp}</div>
                    <div class='info-container'>
                        <div class='name-line'>{name_str} {room_html}</div>
                        {desc_html}
                    </div>
                </div>
            """, unsafe_allow_html=True)
