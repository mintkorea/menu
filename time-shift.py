...
with tab1:
    ...
    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
        ...
        <div style="border:2px solid #2E4077; border-radius:10px; padding:10px; text-align:center;">성희<br><b style="color:red;">{final[1]}</b></div>
        <div style="border:2px solid #2E4077; border-radius:10px; padding:10px; text-align:center;">의산A<br><b>{final[2]}</b></div>
        <div style="border:2px solid #2E4077; border-radius:10px; padding:10px; text-align:center;">의산B<br><b>{final[3]}</b></div>
    </div>
    ''', unsafe_allow_html=True)

with tab2:
    st.markdown("<h3 style='text-align:center;'>📅 C조 상세 리스트</h3>", unsafe_allow_html=True)
    t_html = '<table style="width:100%; border-collapse:collapse; text-align:center; font-size:14px;">'
    ...
    for i in range(15):
        ...
        color = "color:red;" if wd==6 else ("color:blue;" if wd==5 else "")
        t_html += f'<tr><td style="{color}">{day_str}</td><td>{ws[0]}</td><td>{ws[1]}</td><td>{ws[2]}</td><td>{ws[3]}</td></tr>'
    st.markdown(t_html + "</table>", unsafe_allow_html=True)

with tab3:
    hi = st.selectbox("🎯 강조할 조", ["없음", "A", "B", "C"], index=3)
    COLOR_MAP = {
        ...
        for week in cal:
            ...
            for i, day in enumerate(week):
                if day == 0: html += "<td style='background:#f4f4f4;'></td>"
                    ...
                    html += f"<td class='cal-td {today_cls}' style='background-color:{bg};'><span class='date-num' style='color:{d_color};'>{day}</span><span class='shift-name' style='color:{t_color};'>{s_lbl}</span></td>"
            html += "</tr>"
