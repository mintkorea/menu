...
# --- [1] 설정 및 CSS (탭 위치 하향 및 스타일) ---
st.set_page_config(page_title="C조 통합 근무 시스템", layout="wide")

st.markdown("""
    <style>
    /* 탭이 글자 높이만큼 더 내려오도록 여백 추가 */
    .block-container { padding-top: 4rem !important; max-width: 500px; margin: auto; }
    ...
    .table-scroll-container { width: 100%; max-height: 400px; overflow-y: auto; border: 1px solid #dee2e6; border-radius: 5px; }
...
with tab1:
    st.markdown(f'<p style="text-align:right; font-size:11px; color:gray; margin-bottom:5px;">{now.strftime("%Y-%m-%d %H:%M")}</p>', unsafe_allow_html=True)
...
with tab2:
    st.subheader("📅 근무 일정 조회")
    # 복구된 기능: 달력, 슬라이더, 하이라이트 선택
