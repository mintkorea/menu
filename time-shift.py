...
# --- 4. 메뉴별 로직 ---
today_val = datetime.now().date() # NameError 방지 위해 미리 정의

if menu == "📍 실시간 상황판":
    st.markdown("### 📍 실시간 근무 및 안내")
    
    # [핵심] 근무 시간표(Time Table) 고정 표출
    st.info("🕒 **C조 표준 근무 시간표**")
    time_data = {
        "구분": ["주간 근무", "야간 근무", "교대 시간"],
        "시간": ["08:00 ~ 18:00", "18:00 ~ 08:00", "08:00 (정시 교대)"],
        "비고": ["회관/의산 정위치", "야간 순찰 및 대기", "인수인계 철저"]
    }
    ...
    if workers:
        ...
        next_d = today_val + timedelta(days=(3 - (today_val - PATTERN_START_DATE).days % 3) % 3)
        st.info(f"다음 근무 예정일: **{next_d.strftime('%m/%d')}**")

elif menu == "📅 근무 편성표":
    st.markdown("### 📅 전체 근무 편성표 조회")
    # NameError 해결: today_val 사용
    ...
    with t3:
        if st.text_input("암호", type="password") == ADMIN_PASSWORD:
            if not df_vac.empty:
                idx = st.number_input("ID", 0, len(df_vac)-1)
                if st.button("삭제"):
