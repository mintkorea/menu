...
with tab1:
    ...
    if (is_yesterday_work and not is_today_work and now.hour >= 7) or (not is_yesterday_work and not is_today_work):
        ...
        st.markdown(f'<div class="off-card"><div style="font-size:22px; font-weight:900; color:#C04B41;">오늘은 휴무입니다.</div><div style="margin-top:8px; font-weight:700;">다음 근무는 {nw.strftime("%m월 %d일")}입니다 <br>성의회관 : {nj}, {ns}  <br> 의 산 연 : {na}, {nb}</div></div>', unsafe_allow_html=True)
    elif curr_idx != -1:
