...
with tab1:
    ...
    st.markdown(f'<div class="date-display">{now_kst.strftime("%Y-%m-%d")}({weekdays_list[now_kst.weekday()]}) {now_kst.strftime("%H:%M:%S")}</div>', unsafe_allow_html=True)
    
    is_c_day = (get_shift_simple(today_kst) == "C")
    status_msg = ""; highlight_idx = -1

    # 시계열별 격려 문구 & 하이라이트 결정 (보내주신 로직 반영)
...
with tab2:
    ...
    for i in range(31):
        ...
        if ws:
            wd = d.weekday(); lbl = f"{d.strftime('%m/%d')}({weekdays_list[wd]})"; cls = "sun" if wd==6 else ("sat" if wd==5 else "")
