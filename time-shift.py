...
st.markdown("""
    ...
    .today-border { outline: 3px solid #333 !important; outline-offset: -3px; z-index: 10; }
    </style>
...
with tab1:
    ...
    if is_c_day and 6 <= hr < 7:
        ...
        # 현재 시간이 오늘 근무 시간 내(07:00 ~ 07:00 익일)인지 확인하는 세부 로직이 필요할 수 있으나 기존 로직 유지
        status_msg = "😴 오늘은 휴무일이거나 근무 대기 중입니다."
    ...
    h_date = today_kst if is_c_day else NEXT_WORK_DATE
    names = get_workers(h_date)
    h_names = names if names else ["조장", "성희", "당직A", "당직B"]
    
    # 실시간 행 하이라이트 (현재 시간에 맞춰 highlight_idx 계산 로직 추가 가능)
...
with tab2:
    ...
    for i in range(31):
        ...
        ws = get_workers(d)
        if ws:
            wd = d.weekday(); lbl = f"{d.strftime('%m/%d')}({weekdays_short[wd]})"
...
with tab3:
    ...
    # 초기 로드 시 현재 조 선택
    default_choice = curr_logic_shift if curr_logic_shift in options else "선택 없음"
    hi = st.selectbox("🎯 강조 조 선택", options, index=options.index(default_choice))
    
    B_COLS = {"A":"#FFE0B2", "B":"#FFCDD2", "C":"#BBDEFB"}
    S_COLS = {"A":"#FB8C00", "B":"#E53935", "C":"#1E88E5"}
    
    calendar.setfirstweekday(calendar.SUNDAY) # 달력 시작 요일을 일요일로 설정 (핵심 수정)
    cal_html = ""
    ...
    for _ in range(12):
        ...
        cal_html += f"<div style='text-align:center; font-weight:900; font-size:18px; margin-top:20px; margin-bottom:8px;'>{y}년 {m}월</div>"
        ...
        for week in cal:
            ...
            for i, day in enumerate(week):
                if day == 0:
                    cal_html += "<td class='cal-td' style='background:#fcfcfc;'></td>"
                else:
                    d_obj = date(y, m, day)
                    ...
                    d_bg = S_COLS[s] if is_hi else "transparent"
                    
                    td_cls = "today-border" if d_obj == today_kst else ""
                    # i가 0이면 일요일(sun), 6이면 토요일(sat)
