...
with tab3:
    ...
    # 일요일 시작으로 달력 강제 설정 (calendar.setfirstweekday(6))
    calendar.setfirstweekday(6)
    hi = st.selectbox("🎯 강조 조 선택", ["선택 없음", "A", "B", "C"], index=options.index(curr_shift) if 'options' in locals() else 3)
    
    B_COLS, S_COLS = {"A":"#FFE0B2","B":"#FFCDD2","C":"#BBDEFB"}, {"A":"#FB8C00","B":"#E53935","C":"#1E88E5"}
    cal_html = ""; curr_cal = today_kst.replace(day=1)
    for _ in range(12):
        y, m = curr_cal.year, curr_cal.month; cal = calendar.monthcalendar(y, m)
        ...
        for week in cal:
            ...
            cal_html += "</tr>"
        cal_html += "</table>"; curr_cal = (curr_cal.replace(day=1) + timedelta(days=32)).replace(day=1)
