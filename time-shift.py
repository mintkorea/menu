...
with tab2:
    ...
    with c1: start_d = st.date_input("📅 시작일", now.date(), key="cal_d")
    with c2: dur = st.slider("📆 일수", 7, 60, 31, key="cal_s")
    with c3: focus = st.selectbox("👤 강조", ["안 함", "황재업", "김태언", "이태원", "이정석"], key="cal_sb")
