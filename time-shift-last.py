...
with tab1:
    ...
    st.markdown('<div class="status-msg-box">😴 오늘은 휴무일입니다. 편안한 휴식 되세요.</div>', unsafe_allow_html=True)
...
with tab3:
    ...
    with c2:
        hi_shift = st.selectbox("🎯 강조 조", ["A", "B", "C", "없음"], index=0)
    
    # [수동 달력 생성 로직] 2026년 고정
    year = 2026
    first_day = date(year, sel_month, 1)
    # 일요일=0, 월요일=1 ... 토요일=6 인덱스 계산
    ...
    if sel_month == 12:
        ...
        last_day = (date(year, sel_month + 1, 1) - timedelta(days=1)).day
